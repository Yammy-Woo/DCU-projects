...
abstract class Item {
    ...
    int expiryDays; // expiry is a function of days rather than specific days
...
class EasterSale implements OnSale {
    ...
    EasterSale(double salePercent, double minimumAmount) {
        ...
        this.message = "Spend over " + (int)this.minimumAmount + ", Get " + (this.salePercent*100) + "% off for Easter!";
    }
    ...
    public static double billItems(Item[] items, OnSale Sale) {
        ...
        for(int i = 0; i < items.length; i++) {
            if(items[i] instanceof StorageCondition) {
                StorageCondition item = (StorageCondition) items[i];
                System.out.format("%s (%.1f) (Storage: %s)\n", items[i], items[i].price, item.storageProcedure());
            }
            else {
